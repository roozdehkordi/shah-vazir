...
namespace NoSuchStudio.Localization {
    ...
    public partial class LocalizationService : Service<LocalizationService> {
        ...
        // Visible for editor access.
        public Dictionary<string, Dictionary<string, ITranslationSource>> translationSources {
            get { return _translationSources; }
        }
        ...
        private bool SelectLocale(string localeName, bool useNeutralIfFailed = true, bool useDefaultIfFailed = true) {
            if (!_localeDatabase.ContainsLocale(localeName)) {
                LogWarn($"locale {localeName} not in db");
            ...
            if (_locales.Contains(locale)) {
                LogLog($"locale {localeName} is enabled, switching...");
                SetCurrentLocale(localeName);
                return true;
            } else if (useNeutralIfFailed && !locale.IsNeutral && _locales.Contains(locale.Parent)) {
                LogLog($"locale {localeName} is not enabled but its neutral locale {locale.Parent} is, switching...");
                SelectLocale(locale.Parent, true, useDefaultIfFailed);
                return true;
            } else {
                LogLog($"locale {localeName} is not enabled");
                if (useDefaultIfFailed) SelectLocale(_defaultLocale, true, false);
                return false;
            }
        }
        ...
        private bool DoAddLocalizationSource(string phrase, string locale, ITranslationSource source) {
            ...
            if (curPhrase.ContainsKey(locale) && curPhrase[locale] != source) {
                LogWarn($"Multiple localization sources for phrase {phrase}, locale {locale}, old source: {curPhrase[locale].mono.name}, new source: {source.mono.name}");
            }
        ...
        private string DoGetPhraseTranslation(string phrase) {
            return DoGetPhraseTranslation(phrase, _currentLocale);
        }

        private string DoGetPhraseTranslation(string phrase, Locale locale) {
            ...
            if (string.IsNullOrEmpty(phrase)) {
                ...
                return "Error: locale is null or empty";
                ...
                return string.Format("Error: phrase '{0} has no translation in {1} nor its neutral locales.", phrase, locale);
            }
        }
        ...
        private void LoadLocalesDB() {
            ...
            stopwatch.Stop();
            LogLog($"Locale database loaded in {stopwatch.ElapsedMilliseconds / 1000f:0.0000} seconds.");
        }
        ...
        public void DoProcessChangedTranslations() {
            _changedTranslations.ToList().ForEach((vt) => {
                if (_currentLocale.Equals(vt.Item2) && _translationChangeEvents.ContainsKey(vt.Item1)) {
        ...
        public void OnValidate() {
            ...
            if (cachedDatabasePreset != _databasePreset) {
                ...
                if (_databasePreset == DatabasePreset.Custom) {
                    var cachedDBAssetName = cachedDBAsset == null ? "null" : cachedDBAsset.name;
                    var newDBAssetName = _localeDatabaseAsset == null ? "null" : _localeDatabaseAsset.name;
                    LogWarn($"DB Asset file change: {cachedDBAssetName} -> {newDBAssetName}");
                    LoadLocalesDB();
                }
            }
